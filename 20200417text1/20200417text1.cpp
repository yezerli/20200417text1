...
int main() {
	...
	Mat src = imread("E:\\Git\\0417\\rim.png", 0); 
	Mat disMat;
	threshold(src, disMat, 100, 255, THRESH_BINARY_INV);
	//通过findContours函数寻找连通域
	vector<vector<Point>> contours;
	vector<Vec4i> hierarchy;
	findContours(disMat, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
	...
	for (int i = 0; i < contours.size(); i++) {
		...
		if (wh_rate < 1.1 && wh_rate > 0.9 && width > 5) {
